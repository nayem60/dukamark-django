                            ...
                            <ul class="nav nav-tabs flex-sm-column justify-content-between" id="productThumbTab" role="tablist">
                                <li class="nav-item" role="presentation">
                                  <button class="nav-link active" id="thumbOne-tab" data-bs-toggle="tab" data-bs-target="#thumbOne" type="button" role="tab" aria-controls="thumbOne" aria-selected="true">
                                      <img src="{{ variant_id.image.url }}" style="width:90px; height:90px;"alt="">
                                  </button>
                                </li>
                                 {% for variant_img in variant_id.variant_images.all%}
                                 <li class="nav-item" role="presentation">
                                  <button class="nav-link" id="thumbTwo-tab" data-bs-toggle="tab" data-bs-target="#thumbTwo{{ forloop.counter }}" type="button" role="tab" aria-controls="thumbTwo" aria-selected="false">
                                    <img src="{{ variant_img.images.url }}" style="width:90px; height:90px;"alt="">
                                  </button>
                                </li>
                                {% endfor %}
                            </ul>
                                ...
                                <div class="tab-content" id="productThumbContent">
                                    <div class="tab-pane fade show active" id="thumbOne" role="tabpanel" aria-labelledby="thumbOne-tab">
                                        <div class="product__details-nav-thumb w-img">
                                            <img src="{{ variant_id.image.url }}" alt="">
                                        </div>
                                    </div>
                                    {% for variant_imgs in variant_id.variant_images.all %}
                                    <div class="tab-pane fade" id="thumbTwo{{ forloop.counter }}" role="tabpanel" aria-labelledby="thumbTwo-tab">
                                        <div class="product__details-nav-thumb w-img">
                                            <img src="{{variant_imgs.images.url}}" alt="">
                                        </div>
                                    </div>
                                    {% endfor %}
                                  </div>
                    ...
                    <div class="col-xl-6">
                        <div class="product__details-nav d-sm-flex align-items-start">
                            <ul class="nav nav-tabs flex-sm-column justify-content-between" id="productThumbTab" role="tablist">
                                <li class="nav-item" role="presentation">
                                  <button class="nav-link active" id="thumbOne-tab" data-bs-toggle="tab" data-bs-target="#thumbOne" type="button" role="tab" aria-controls="thumbOne" aria-selected="true">
                                      <img src="{{ product.image.url }}" style="width:90px; height:90px;"alt="">
                                  </button>
                                </li>
                                {% for images in product.image_gellery.all %}
                                <li class="nav-item" role="presentation">
                                  <button class="nav-link" id="thumbThree-tab" data-bs-toggle="tab" data-bs-target="#thumbThree-{{ forloop.counter}}" type="button" role="tab" aria-controls="thumbThree" aria-selected="false">
                                    <img src="{{ images.images.url }}" style="width:90px; height:90px;" alt="">
                                  </button>
                                </li>
                               {% endfor %}
                            </ul>
                            <div class="product__details-thumb">
                                <div class="tab-content" id="productThumbContent">
                                    ...
                                    {% for images in product.image_gellery.all %}
                                    <div class="tab-pane fade" id="thumbThree-{{ forloop.counter }}" role="tabpanel" aria-labelledby="thumbThree-tab">
                                        <div class="product__details-nav-thumb w-img">
                        ...
                        <div class="product__details-content">
                           ...
                           {% if product.id|size %}
                            <div class="mb-4">
                               <label> Size : </label>
                               {% if variant_id.size %}
                                  <button onclick="size({{variant_id.id}})" class="btn btn-ligth btn-sm border border-warning rounded mr-5 text-warning">{{ variant_id.size.name }}</button>
                               {%else%}
                               
                               {% for size in product.variant.all %}
                                 <button onclick="size({{size.id}})" class="btn btn-ligth btn-sm border border-warning rounded mr-5 text-warning">{{ size.size.name }}</button>
                               {% endfor %}
                            ...
                            {% if product.id|ram%}
                              <div class="mb-4">
                               <label> Ram : </label>
                               {% for ram in product.variant.all %}
                               <button class="btn btn-ligth btn-sm border border-warning rounded mr-5 text-warning">{{ ram.ram.name }}</button>
                               {% endfor %}
                              </div>
                            {% endif %}
                            ...
                            {% if product.id|storage%}
                              <div class="mb-4">
                               <label> Storage : </label>
                               {% for storage in product.variant.all %}
                               <button class="btn btn-ligth btn-sm border border-warning rounded mr-5 text-warning">{{ storage.storage.name }}</button>
                               {% endfor %}
                              </div>
                            {% endif %}
                          ...
                          {% if product.id|variant %}
                            ...
                            <form id="color_form">
                             <label> Color : </label>
                            {% for color in product.variant.all %}
                              {% if color.image %}
                               {% if color.id == color_id %}
                               <a href="javascript:void(0)" onclick="color({{color.id}},1)" class="mr-5">
                                  <img  class="border border-warning" src="{{ color.image.url }}" style="width:50px; height:50px;">
                               </a>
                              {% else %}
                               <a href="javascript:void(0)" onclick="color({{color.id}},0)" class="mr-5">
                                  <img  class="border " src="{{ color.image.url }}" style="width:50px; height:50px;">
                               </a>
                              {% endif %}
                            {% endif %}
                              
                            {% endfor %}
                               <input type="hidden" name="color" id="color" value="{{ color_id }}">
                               <input type="text" name="size" id="size" value="{{ size_id }}">
                              </form>
                            </div>
                            ...
                            <div class="product-stock mb-20">
                                <h5>Availability: <span>  {% if product.quantity >= 1 %} {{product.quantity}} in stock {% else %} <b style="color:red;">Out Stock</b> {% endif%}</span></h5>
                            </div>
